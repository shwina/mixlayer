...
def dfdx(f, dx, bc_type=BoundaryConditionType.PERIODIC, bc_val=0):
    """
    ...
    for i in prange(ny):
        ...
        if bc_type == BoundaryConditionType.PERIODIC:
            for j in range(4):
                out[i,j] = (1./dx)*(
                        ( 4 / 5  ) * (f[i,j+1] - f[i,j-1]) + 
                        (-1 / 5  ) * (f[i,j+2] - f[i,j-2]) +
...
@jit(nopython=True, nogil=True, parallel=True)
def dfdy(f, dy, bc_type=BoundaryConditionType.PERIODIC, bc_val=0):
    """
    Compute the derivative of a 2-d array on a regularly spaced grid
